...
namespace AccountSettings{
  ...
  public class AccountSettingsManager : MonoBehaviour{
    ...
    [SerializeField] private AccountSettings accountSettings;
    ...
    /***************************************/
    private void Awake(){
      _accountSettingsCaretaker=new AccountSettingsCaretaker(accountSettings);
      ...
      searchUsernameButton.onClick.AddListener(() => {
        ...
        if(found)
          ...
          JsonUtility.FromJson<AccountSettingsSerializable>(File.ReadAllText(FolderPath + usernameInputField.text + Extension)).FillAccountSettings(accountSettings);
        else
          //GameSettings with default settings
          JsonUtility.FromJson<AccountSettingsSerializable>(File.ReadAllText(FolderPath+DefaultFilename+Extension)).FillAccountSettings(accountSettings); //end-if
      });
    }
    /***************************************/
  }
