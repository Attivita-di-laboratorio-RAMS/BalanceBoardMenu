...
namespace AccountSettings
{
    ...
    public class AccountSettingsSerializable
    {
        ...
        public AccountSettingsSerializable(AccountSettings accountSettings)
        {
            ...
            {
                defaultMovementsCheckboxesList.Add(new CustomString(
                    accountSettings.GetDefaultMovementCheckboxesList()[i].isOn.ToString(),
                    accountSettings.GetDefaultMovementCheckboxesList()[i].interactable.ToString()));
            } //end-for
            ...
            for (var i = 0; i < accountSettings.GetDefaultMovementAmpInputFieldsList().Count; i += 1)
            {
                defaultMovementsAmpInputFieldsList.Add(new CustomString(
                    ...
                    accountSettings.GetDefaultMovementAmpInputFieldsList()[i].interactable.ToString()));
            } //end-for
            ...
            for (var i = 0; i < accountSettings.GetDefaultMovementFreqInputFieldsList().Count; i += 1)
            {
                defaultMovementsFreqInputFieldsList.Add(new CustomString(
                    ...
                    accountSettings.GetDefaultMovementFreqInputFieldsList()[i].interactable.ToString()));
            } //end-for
        }
        ...
        public void FillAccountSettings()
        {
            ...
            for (var i = 0; i < defaultMovementsCheckboxesList.Count; i += 1)
            {
                accountSettings.GetDefaultMovementCheckboxesList()[i].isOn =
                    Convert.ToBoolean(defaultMovementsCheckboxesList[i].s1);
                accountSettings.GetDefaultMovementCheckboxesList()[i].interactable =
                    Convert.ToBoolean(defaultMovementsCheckboxesList[i].s2);
            } //end-for
